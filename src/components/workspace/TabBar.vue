...
import { useSidebarWidth } from "@/services/sidebarWidth";
...
const sidebarWidth = useSidebarWidth("file");
const tabContainerStyle = computed(() => ({
  marginLeft: shouldOffsetTabBar.value ? `${sidebarWidth.value}px` : "0px",
}));

const isMacPlatform = ref(false);
onMounted(async () => {
  try {
    isMacPlatform.value = (await osPlatform()) === "macos";
  } catch {
    isMacPlatform.value = false;
  }
});

// 拦截 ctrl/cmd + w 快捷键关闭tab
function handleCloseTabShortcut(e: KeyboardEvent) {
  if ((isMacPlatform.value ? e.metaKey : e.ctrlKey) && e.key.toLowerCase() === "w") {
    e.preventDefault();
 ...
 * 单 Tab：直接进入窗口拖拽模式（由 Tauri 宿主侧持续驱动位置更新）
 */
