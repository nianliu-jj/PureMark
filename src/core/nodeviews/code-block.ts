...
export class CodeBlockView implements NodeView {
  ...
  constructor(node: ProseMirrorNode, view: ProseMirrorView, getPos: () => number | undefined) {
    ...
    this.dom.className = "puremark-code-block";
    ...
    this.editorContainer.className = "puremark-code-block-editor";
  ...
  private setSourceViewMode(enabled: boolean): void {
    ...
    if (enabled) {
      ...
      if (!this.sourceTextElement) {
        ...
        this.sourceTextElement.className = "puremark-code-block-source-container";
      ...
      lines.forEach((line) => {
        ...
        lineDiv.className = "puremark-with-line-number";
  ...
  private handleSourceTextInput(): void {
    ...
    // 从所有子 div 中收集文本内容
    const lines: string[] = [];
    const childDivs = this.sourceTextElement.querySelectorAll("div.puremark-with-line-number");
      ...
      requestAnimationFrame(() => {
        if (this.sourceTextElement && selection) {
          try {
            ...
            const divs = this.sourceTextElement.querySelectorAll("div.puremark-with-line-number");
  ...
  private createHeader(language: string): HTMLElement {
    ...
    header.className = "puremark-code-block-header";

    // 语言选择器
    const langSelector = this.createCustomSelect(supportedLanguages, language, (value) =>
      this.setLanguage(value)
    );
    langSelector.classList.add("puremark-code-block-lang-select");
    ...
    if (language === "mermaid") {
      ...
      modeSelector.classList.add("puremark-code-block-mode-select");
      header.appendChild(modeSelector);
    }

    // 复制按钮（hover 时显示）
    const copyBtn = document.createElement("button");
    copyBtn.className = "puremark-code-block-copy-btn";
    copyBtn.type = "button";
    copyBtn.title = "复制代码块";
  ...
  private createCustomSelect(
    ...
    container.className = "puremark-custom-select";

    const button = document.createElement("button");
    button.className = "puremark-custom-select-button";
    button.type = "button";
    const currentOption = options.find((o) => o.value === currentValue);
    button.textContent = currentOption?.label || options[0].label;

    const dropdown = document.createElement("div");
    dropdown.className = "puremark-custom-select-dropdown";

    for (const option of options) {
      const item = document.createElement("div");
      item.className = "puremark-custom-select-item";
      ...
      item.addEventListener("click", (e) => {
        ...
        // 更新选中状态
        dropdown.querySelectorAll(".puremark-custom-select-item").forEach((el) => {
    ...
    button.addEventListener("click", (e) => {
      e.stopPropagation();
      // 关闭所有其他下拉框
      document.querySelectorAll(".puremark-custom-select.open").forEach((el) => {
        if (el !== container) {
  ...
  private async showContextMenu(e: MouseEvent): Promise<void> {
    ...
    const isHeaderContextMenu = !!(e.target instanceof HTMLElement
      ? e.target.closest(".puremark-code-block-header")
      : null);
    ...
    separator.className = "puremark-context-menu-separator";
  ...
  private async createMermaidPreview(content: string): Promise<void> {
    if (!this.mermaidPreview) {
      ...
      this.mermaidPreview.className = "puremark-mermaid-preview";
    ...
    mermaidRenderQueue = mermaidRenderQueue.then(async () => {
      ...
      try {
        ...
        preview.innerHTML = `<div class="puremark-mermaid-error">Mermaid 渲染错误</div>`;
      }
    });
  ...
  stopEvent(event: Event): boolean {
    ...
    if (event.target instanceof HTMLElement) {
      const isInHeader = event.target.closest(".puremark-code-block-header");
      if (isInHeader) {
        return false;
      }
    }
